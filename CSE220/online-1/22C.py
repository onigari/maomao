...
def time_reverse(x: np.ndarray) -> np.ndarray:
    ...
    The time axis t = linspace(T_MIN, T_MAX, N) is symmetric about 0
    (T_MIN = -T_MAX) and includes 0 as its center sample (N is odd),
    so t[N-1-i] == -t[i] for every i. That means simply reversing the
    sample array gives exactly the samples of x(-t) on the same grid.
    """
    return x[::-1].copy()
